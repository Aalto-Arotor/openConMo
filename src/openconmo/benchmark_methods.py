...
def envelope(signal, fs):
    ...
    This method applies envelope analysis using the squared envelope
    spectrum of the full-bandwidth signal. It is suitable when there are
    no strong masking sources in the signal, and bearing faults can be
    diagnosed directly. Often used as a simple baseline in fault detection.
...
def cepstrum_prewhitening(signal, fs):
    ...
    This method performs signal pre-whitening using cepstrum editing,
    as described in "Signal Pre-whitening Using Cepstrum Editing
    (Liftering) to Enhance Fault Detection in Rolling Element Bearings."
    ...
    # Compute the log-amplitude spectrum, avoiding log(0)
    log_amplitude = np.log(np.abs(X) + 1e-10)

    # Compute real cepstrum via IFFT of log-amplitude spectrum
    real_cepstrum = scipy.fft.ifft(log_amplitude)

    # Initialize edited cepstrum - keep only DC component
    ...
    # Reconstruct the edited log spectrum using original phase
...
def DRS(signal, N, Delta):
    ...
    by constructing a frequency-domain filter based on delayed
    cross-power spectra. The approach is based on:
    "Unsupervised Noise Cancellation for Vibration Signals: Part II–
    A Novel Frequency-Domain Algorithm."
    https://doi.org/10.1016/S0888-3270(03)00013-X
    ...
    deterministic_part : ndarray
        The reconstructed deterministic (periodic/discrete frequency)
        part of the signal.

    """
    # Initialize numerator and denominator for frequency-domain filter
    nominator = np.zeros(N, dtype=complex)
    denominator = np.zeros(N, dtype=complex)

    # Estimate cross- and power spectra across the signal
    for k in tqdm(
        range(2 * N + Delta, len(signal), 100), desc="Building DRS filter"
    ):
        ...
        # Compute FFTs of the current and delayed windows
    ...
    h_n = np.flip(h_n)  # Flip the filter for convolution
    deterministic_part = scipy.signal.fftconvolve(signal, h_n)[: len(signal)]
    random_part = signal - deterministic_part
...
def benchmark_method(signal, fs, N=16384, Delta=500, nlevel=2):
    """
    Fault detection using DRS, kurtogram-based filtering, and envelope.
    ...
    This method combines three steps for enhancing impulsive features
    in vibration signals:
    1. Discrete/Random Separation (DRS) to remove deterministic
       frequency components.
    2. Spectral kurtosis analysis via a fast kurtogram to select the
       most impulsive frequency band.
    3. Envelope analysis (squared envelope spectrum) of the filtered
       signal to extract fault signatures.
    ...
    nlevel : int, optional
        Number of decomposition levels used in the fast kurtogram
        (default is 2).
    ...
    t = np.linspace(
        (N + Delta) / fs, len(signal) / fs, len(signal) - N - Delta
    )

    # Step 2: Use fast kurtogram to find most impulsive frequency band
    _, _, _, fc, bandwidth = fast_kurtogram(
        filtered_signal, fs, nlevel=nlevel, verbose=True
    )
...
if __name__ == "__main__":
    msg = (
        "This module contains benchmark methods for fault detection "
        "in vibration signals."
    )
    print(msg)
