...
def read_from_parquet(contents):
    ...
    # Extract metadata
    fs = df.attrs['sampling_frequency']
    name = df.attrs['name']
    measurement_location = df.attrs['measurement_location']
    fault_frequencies = df.attrs.get('fault_frequencies', None)  # Returns None if not found
    rotating_freq_hz = df.attrs['rotating_freq_hz']
    
    return signal, fs, name, measurement_location, unit, fault_frequencies, rotating_freq_hz

# def read_from_parquet(filename):
#     """
#     Read measurement data from a parquet file.
...
#     return signal, fs, name, measurement_location, unit, fault_frequencies